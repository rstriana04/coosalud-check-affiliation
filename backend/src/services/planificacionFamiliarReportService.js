...
import { PlanificacionFamiliarDataExtractor } from '../utils/planificacionFamiliarDataExtractor.js';
import { generatePlanificacionFamiliarExcel } from '../utils/planificacionFamiliarExcelGenerator.js';
...
export class PlanificacionFamiliarReportService {
  constructor() {
  ...
  async generateFromExcel(inputExcelPath, options = {}) {
    ...
    try {
      ...
      this.emitLog(jobId, 'info', 'Generando Excel planificacion familiar...');
      const outputDir = this.ensureOutputDir();
      const timestamp = this.timestamp();
      const filePrefix = `planificacion-familiar-${this.today()}`;

      const excelPath = join(outputDir, `${filePrefix}-${timestamp}.xlsx`);
      await generatePlanificacionFamiliarExcel(successData, excelPath);
      ...
      logger.error('Error generando informe planificacion familiar', {
        error: error.message
      });
    ...
    for (let i = 0; i < patients.length; i++) {
      ...
      if (lastError) {
        ...
        results.push({
          ...
          fecha: patient.fecha_atencion, programa: 'planificacion-familiar',
  ...
  async processOnePatient(patient, logPrefix, jobId) {
    ...
    this.emitLog(jobId, 'info',
      `${logPrefix} Buscando cita del ${patient.fecha_atencion}...`
    );
    const codCita = await this.scraper.filterAndExtractCodCita(
      patient.fecha_atencion
    );

    if (!codCita) {
      throw new Error(
        `CodCita no encontrado para fecha ${patient.fecha_atencion}`
      );
    }
    ...
    const extractor = new PlanificacionFamiliarDataExtractor(pdfPath);
