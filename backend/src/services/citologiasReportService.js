...
import { CitologiasDataExtractor } from '../utils/citologiasDataExtractor.js';
import { generateCitologiasExcel } from '../utils/citologiasExcelGenerator.js';
...
export class CitologiasReportService {
  constructor() {
  ...
  async generateFromExcel(inputExcelPath, options = {}) {
    ...
    try {
      ...
      this.emitLog(jobId, 'info', 'Generando Excel citologias...');
      const outputDir = this.ensureOutputDir();
      const timestamp = this.timestamp();
      const filePrefix = `citologias-${this.today()}`;

      const excelPath = join(outputDir, `${filePrefix}-${timestamp}.xlsx`);
      await generateCitologiasExcel(successData, excelPath);
      ...
      logger.error('Error generando informe citologias', {
        error: error.message
      });
    ...
    for (let i = 0; i < patients.length; i++) {
      ...
      if (lastError) {
        ...
        results.push({
          ...
          fecha: patient.fecha_atencion, programa: 'citologias',
  ...
  async processOnePatient(patient, logPrefix, jobId) {
    ...
    const extractor = new CitologiasDataExtractor(pdfPath);
    ...
    try {
      ...
      const res202Record = mapToResolucion202(extractedData, 'citologias', reportingPeriod);
