...
import { LifecycleDataExtractor } from '../utils/lifecycleDataExtractor.js';
import { generateLifecycleExcel } from '../utils/lifecycleExcelGenerator.js';
...
const VALID_PROGRAMS = ['juventud', 'adultez', 'vejez'];
...
export class LifecycleReportService {
  ...
  async generateFromExcel(inputExcelPath, options = {}) {
    ...
    try {
      ...
      this.emitLog(jobId, 'info', 'Generando Excel ciclo de vida...');
      const outputDir = this.ensureOutputDir();
      const timestamp = this.timestamp();
      const filePrefix = `ciclovida-${this.today()}`;

      const excelPath = join(outputDir, `${filePrefix}-${timestamp}.xlsx`);
      const excelFilePath = await generateLifecycleExcel(dataByProgram, excelPath);
      ...
      const zipFilePath = await this.scraper.createZipArchive(pdfFiles, excelFilePath, zipPath);
      ...
      logger.error('Error generando informe ciclo de vida', { error: error.message });
    ...
    for (let i = 0; i < validPatients.length; i++) {
      ...
      if (lastError) {
        ...
        results.push({
          ...
          fecha: patient.fecha_atencion, programa,
      ...
      this.emitProgress(jobId, {
        total: validPatients.length, processed: i + 1,
  ...
  async processOnePatient(patient, programa, logPrefix, jobId) {
    ...
    const extractor = new LifecycleDataExtractor(pdfPath, programa);
    ...
    return {
      rowNumber: patient.rowNumber, identipac: patient.identipac,
      fecha: patient.fecha_atencion, programa, patientId, codCita,
      pdfPath, extractedData, status: 'success'
    };
  }
  ...
  async recoverFromError() {
    try {
      ...
      if (!page || page.isClosed()) { await this.reinitializeScraper(); return; }
      await page.goto(this.scraper.baseUrl + 'administrador/', {
        waitUntil: 'networkidle', timeout: 30000
      });
      const menuVisible = await page.$('#menu2');
      if (!menuVisible) { await this.reinitializeScraper(); return; }
    } catch (_) {
